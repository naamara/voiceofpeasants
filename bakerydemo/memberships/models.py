...
from bakerydemo.memberships.choices import DAY_CHOICES
...
class MembershipOperatingHours(Orderable, OperatingHours):
    """
...
class MembershipsIndexPage(Page):
    """
    ...
    def get_context(self, request):
        context = super(MembershipsIndexPage, self).get_context(request)
        context['locations'] = MembershipPage.objects.descendant_of(
            self).live().order_by(
            'title')
        return context
...
class MembershipPage(Page):
    ...
    def is_open(self):
        ...
        try:
            ...
            return True
        except MembershipOperatingHours.DoesNotExist:
            return False
    ...
    parent_page_types = ['MembershipsIndexPage']
